...
typedef ibitwrap<uint8_t>  ibitwrap8;
typedef obitwrap<uint8_t>  obitwrap8;
typedef ibitwrap<uint16_t> ibitwrap16;
typedef obitwrap<uint16_t> obitwrap16;
typedef ibitwrap<uint32_t> ibitwrap32;
typedef obitwrap<uint32_t> obitwrap32;
typedef ibitwrap<uint64_t> ibitwrap64;
typedef obitwrap<uint64_t> obitwrap64;
//@}
...
public:
	...
	virtual size_t pos(size_t newpos) {
        _understream->pos(newpos / maxbits());
        _bits = newpos % maxbits();
		return newpos;
	}
	//@}
...
protected:
	...
	/// Bitmask.
	static B bitmask(bc_t bit) { return B(1) << bit; }
