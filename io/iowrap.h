...
public:
	...
	iwrapelem(istream<T2>& in_str):
	  iwrap<T, T2>(in_str), _convert(default_convert) 
	{}
	
	iwrapelem(istream<T2>& in_str, convert_func f):
	  iwrap<T, T2>(in_str), _convert(f) 
	{}
...
protected:
    ...
    virtual void convert(const T2& x, T& y) const {
        _convert(x, y);
    }
...
public:
	...
	owrapelem(ostream<T2>& out_str):
	  owrap<T, T2>(out_str), _convert(default_convert) {}
	
	owrapelem(ostream<T2>& out_str, convert_func f):
	  owrap<T, T2>(out_str), _convert(f) {}
...
private:
    ...
    virtual void convert(const T& x, T2& y) const {
        _convert(x, y);
    }
