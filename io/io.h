...
public:
    ...
    virtual istream<T1>& input() = 0;
    virtual ostream<T2>& output() = 0;
	operator istream<T1>&() { return input();  }
	operator ostream<T2>&() { return output(); }
	//@}

};

/// 
/// Abstract input-output filter.
/// Inputs some data, transforms it somehow, and outputs.
///
template<typename T1, typename T2 = T1>
class filter
{
public:
    ...
    virtual size_t execute(istream<T1>& input, ostream<T2>& output) const = 0;
