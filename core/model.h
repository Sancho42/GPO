...
inline freq_t critical_band(freq_t f) { 
	return f_alpha * f + f_beta;
}

/// Коэффициент пропорциональности в 1/мм.
const double f_C = log(critical_band(e_Fhigh) / critical_band(e_Flow));

/// Координата точки на основной мембране в мм для заданной частоты анализа f
inline double f2x(freq_t f) { 
	return e_Xm * log(critical_band(e_Fhigh) / critical_band(f)) / f_C;
}

/// Частота анализа для заданной координаты точки x на основной мембране
inline freq_t x2f(double x) { 
	return (critical_band(e_Fhigh) / exp(x / e_Xm * f_C) - f_beta) / f_alpha;
}

/// Добротность фильтра с резонансной частотой f
inline double filter_quality(freq_t f) {
	return f_B * f / critical_band(f);
}
...
inline double filter_std(freq_t Fr, freq_t Fs) {
	return f_C24 * M_SQRT1_2 * filter_quality(Fr) * Fs / (2 * M_PI * Fr);
}
...
//	return Fr / (f_C24 * filter_quality(Fr) * delta);
	return critical_band(Fr) / (f_C24 * f_B * delta);
}
